...
const networks = ['Ethereum', 'Base', 'Robinhood Chain'];

export default function Dashboard() {
  const [network, setNetwork] = useState(networks[0]);
  const [connected, setConnected] = useState(false);
  return <main className="dashboard">
    <header className="dash-header"><a href="/" className="dash-brand">DEPROTECTOR</a><nav><a href="/dashboard?view=phishing">Phishing shield</a><a href="/dashboard?view=revocation">Auto-revoke</a><a className="active" href="/dashboard">Dashboard</a></nav><button className="connect" onClick={() => setConnected(!connected)}>{connected ? 'Wallet connected' : 'Connect wallet'}</button></header>
